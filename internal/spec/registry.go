...
func (r *Registry) List(ctx context.Context, scope domain.Scope, allVersions bool) ([]domain.AgentSummary, error) {
	// DISTINCT ON is why the default is one row per agent: picking the newest
	// version in SQL beats reading every version and discarding most of them.
	selection := `
		select distinct on (agent_id)
		       agent_id, version_id, company_id, area_id, name,
		       provider, model, effort, tools, budget, triggers,
		       published_by, published_at, true
		from agent_specs %s
		order by agent_id, published_at desc`
	if allVersions {
		selection = `
			select agent_id, version_id, company_id, area_id, name,
			       provider, model, effort, tools, budget, triggers,
			       published_by, published_at,
			       published_at = max(published_at) over (partition by agent_id)
			from agent_specs %s
			order by agent_id, published_at desc`
	}
...
func scopeSQL(scope domain.Scope) (string, []any) {
	...
	if scope.Company != "" {
		...
		clauses = append(clauses, fmt.Sprintf("company_id = $%d", len(args)))
	}
	if scope.Area != "" {
		args = append(args, string(scope.Area))
		clauses = append(clauses, fmt.Sprintf("area_id = $%d", len(args)))
	}
	if len(clauses) == 0 {
		return "", nil
	}
