...
describe("what a step says it did", () => {
  it("reads the Gate's verdict rather than the step kind", () => {
    ...
    expect(verbOf(step({ payload: { verdict: 4 } })).verb).toBe("bloqueou");
    expect(verbOf(step({ payload: { verdict: 1 } })).verb).toBe("permitiu");
  });

  it("falls back to the kind when the verdict is missing", () => {
    expect(verbOf(step({ payload: {} })).verb).toBe("decidiu");
  });
