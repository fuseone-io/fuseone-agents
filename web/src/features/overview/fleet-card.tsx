  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <StateDot state={stateOfAgent(activity?.lastPhase as Phase | undefined)} />
        <div className="min-w-0 flex-1">
          <div className="truncate text-sm font-medium">{agent.name}</div>
          <div className="truncate text-xs text-muted-foreground">{agent.scope.area || "—"}</div>
        </div>
      </div>
      ...
      {trend.some(Boolean) ? (
        <Sparkline points={trend} width={200} height={28} className="w-full text-primary" />
      ) : (
      ...
      <div className="flex flex-wrap gap-3.5 font-mono text-2xs tabular-nums text-muted-foreground">
        <span>
          <span className="text-foreground">{runs}</span> exec.
        </span>
        <span>
          <span className="text-foreground">{successRate(agent)}</span> ok
        </span>
        <span>
