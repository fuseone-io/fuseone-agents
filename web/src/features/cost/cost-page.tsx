      ...
      {error ? (
        ...
        <>
          <div className="flex flex-wrap gap-4">
            <Panel title="Gasto · últimos 14 dias" className="min-w-[320px] flex-[2_1_380px]">
              <BarChart
            ...
            <Panel title="De onde vem o custo" className="min-w-[260px] flex-[1_1_280px]">
            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  ...
                  <TableHead className={`${HEAD} text-right`}>Execuções</TableHead>
                  <TableHead className={`${HEAD} text-right`}>Médio</TableHead>
                  <TableHead className={`${HEAD} text-right`}>Tokens</TableHead>
                ...
                {buckets.map((b) => (
                  <TableRow key={b.key} className="h-10 border-border-subtle">
                    ...
                    <TableCell className={NUM}>
                      {b.runs === 0 ? "—" : formatMicros(Math.round(b.cost.micros / b.runs))}
                    </TableCell>
                    <TableCell className={NUM}>
                      <Mono dim>{formatTokens((b.cost.inputTokens ?? 0) + (b.cost.outputTokens ?? 0))}</Mono>
                    </TableCell>
                    <TableCell className={NUM}>{formatMicros(b.cost.micros)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
