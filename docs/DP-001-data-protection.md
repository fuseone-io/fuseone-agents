...
| `run_steps` | Who asked, which agent, which tool, what the Gate decided; data-flow labels for company, area, untrusted or personal origin; references and digests for bulk content and shared context artifacts — **and the free text listed in section 1**, permanently |
...
run outside that scope. They are also what makes shared context between agents
auditable by reference instead of by copied prose: the ledger carries artifact
name, source run, digest and labels, while the artifact bytes remain in
`run_content`.
